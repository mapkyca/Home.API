...
namespace home_api\core {
    ...
    class SubsystemFactory {
        ...
        public static function factory($label) {
            ...
            if (!$subsystem)
                throw new \home_api\core\exceptions\SubsystemFactoryException("Subsystem Factory does not know how to make '$label'");

            $reflect = new \ReflectionClass($subsystem['class']);
            if (isset($subsystem['parameters']))
                return \home_api\core\Events::trigger("factory:$label", 'construct', array('return' => $reflect->newInstanceArgs($subsystem['parameters'])));

            return \home_api\core\Events::trigger("factory:$label", 'construct', array('return' => $reflect->newInstance()));
        }
